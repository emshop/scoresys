	...
	<div class="panel panel-default">
		...
		<div class="panel-body" id="panel-body">
			<el-form ref="form" :inline="true" class="form-inline pull-left">
				<el-form-item>
					<el-input clearable size="medium" v-model="queryData.pn_name" placeholder="请输入名称">
					</el-input>
				</el-form-item>
		...
		<el-scrollbar style="height:100%">
			<el-table :data="dataList.items" stripe style="width: 100%" :max-height="maxHeight">
				...
				<el-table-column   prop="pn_id" label="编号" align="center">
				<template slot-scope="scope">
					<span>{{scope.row.pn_id}}</span>
				</template>
				
				</el-table-column>
				<el-table-column   prop="pn_name" label="名称" align="center">
					<template slot-scope="scope">
						<el-tooltip class="item" v-if="scope.row.pn_name && scope.row.pn_name.length > 20" effect="dark" placement="top">
							<div slot="content" style="width: 110px">{{scope.row.pn_name}}</div>
							<span>{{scope.row.pn_name | fltrSubstr(20) }}</span>
						</el-tooltip>
						<span v-else>{{scope.row.pn_name}}</span>
					</template>
				</el-table-column>
...
<script>
import Add from "./punishment.info.add"
import Edit from "./punishment.info.edit"
export default {
  components: {
		Add,
		Edit
  ...
  mounted(){
    ...
    query(){
      ...
      let res = this.$http.xpost("/punishment/info/query",this.$utility.delEmptyProperty(this.queryData))
    ...
    resetForm(formName) {
      ...
      this.$emit("addTab","详情"+val.pn_id,"/punishment/info/detail",data);
		},
    showAdd(){
      this.$refs.Add.show();
		},
