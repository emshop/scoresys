	...
	<div class="panel panel-default">
		...
		<div class="panel-body" id="panel-body">
			<el-form ref="form" :inline="true" class="form-inline pull-left">
				<el-form-item>
					<el-input clearable size="medium" v-model="queryData.gif_name" placeholder="请输入礼品名称">
					</el-input>
				</el-form-item>
		...
		<el-scrollbar style="height:100%">
			<el-table :data="dataList.items" stripe style="width: 100%" :max-height="maxHeight">
				...
				<el-table-column   prop="gif_id" label="编号" align="center">
				<template slot-scope="scope">
					<span>{{scope.row.gif_id}}</span>
				</template>
				
				</el-table-column>
				<el-table-column   prop="gif_name" label="礼品名称" align="center">
					<template slot-scope="scope">
						<el-tooltip class="item" v-if="scope.row.gif_name && scope.row.gif_name.length > 20" effect="dark" placement="top">
							<div slot="content" style="width: 110px">{{scope.row.gif_name}}</div>
							<span>{{scope.row.gif_name | fltrSubstr(20) }}</span>
						</el-tooltip>
						<span v-else>{{scope.row.gif_name}}</span>
					</template>
				</el-table-column>
...
<script>
import Add from "./gif.info.add"
import Edit from "./gif.info.edit"
export default {
  components: {
		Add,
		Edit
  ...
  mounted(){
    ...
    query(){
      ...
      let res = this.$http.xpost("/gif/info/query",this.$utility.delEmptyProperty(this.queryData))
    ...
    resetForm(formName) {
      ...
      this.$emit("addTab","详情"+val.gif_id,"/gif/info/detail",data);
		},
    showAdd(){
      this.$refs.Add.show();
		},
